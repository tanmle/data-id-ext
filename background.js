...
chrome.runtime.onInstalled.addListener(() => {
  chrome.storage.sync.get(
    { attributeName: 'data-testid', elementTypes: [] },
    (data) => {
      chrome.storage.sync.set({
        attributeName: data.attributeName || 'data-testid',
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  ...
  try {
    const settings = await chrome.storage.sync.get({ attributeName: 'data-testid' });
